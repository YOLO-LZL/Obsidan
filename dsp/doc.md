...
- **定义**：输出信杂比（SCR）与输入信杂比的比值 \[ $I(f_D) = \frac{(\text{SCR})_{\text{out}}}{(\text{SCR})_{\text{in}}}$ \] 
- **意义**：这是更全面的指标，因为它同时考虑了**对目标的增益**和**对杂波的衰减**。 
- 对线性接收机，它可以进一步写成： \[ $I(f_D) = \left(\frac{S_o}{S_i}\right) \cdot CA$ \] 这里 \($S_o/S_i$\) 是滤波器对目标信号的增益。 
#### 二、 平均改善因子与杂波衰减的关系 
在很多雷达场景中，目标的多普勒频率具有模糊性，可以看作在PRF范围内均匀分布的随机变量。 
- 此时，**平均信号增益**等于1（因为噪声增益被设计为1），所以平均改善因子就等于杂波衰减：
\[ $I = CA$ \] 
- 这个等式只在**线性接收机**中成立。对于非线性接收机，必须使用改善因子来评估性能，否则杂波衰减会给出误导性结果。 
#### 三、 用帕塞瓦尔定理计算输出杂波功率 
直接用传递函数积分计算输出杂波功率有时比较复杂，文中引入了**帕塞瓦尔定理**，提供了一种更简便的方法： \[ $\int_{-\infty}^{\infty} R_C(\tau) R_h(\tau) d\tau = \int_{-\infty}^{\infty} C(f) |H(f)|^2 df$ \] 
- \($R_C(\tau)$\) 是杂波的自相关函数 
- $R_h(\tau)$ 是滤波器的自相关函数 - 对于高斯杂波谱，自相关函数为： \[ $R_C(\tau) = \exp\left[-2(\pi \sigma_f \tau)^2\right]$ \] 
#### 四、 单延迟对消器的具体计算 
对单延迟对消器（双脉冲对消器），我们可以得到其脉冲响应和自相关函数： 
1. **脉冲响应** $h(t) = \frac{\delta(t) - \delta(t-T)}{\sqrt{2}}$ 
2. **自相关函数** $R_h(\tau) = \frac{2\delta(\tau) - \delta(\tau-T) - \delta(\tau+T)}{2}$  代入帕塞瓦尔定理，输出杂波功率可以简化为： $[ P_{CO} = R_C(0) - R_C(T) ]$ 因此，改善因子为： $[ I = \frac{R_C(0)}{R_C(0) - R_C(T)} ]$ 
#### 五、 高斯杂波谱的近似 
对于高斯杂波谱，当抑制效果较强时（即 \(x\) 很小），可以用 \($\exp(-x) \approx 1-x$\) 近似，得到改善因子的简化公式： \[ $I\approx \frac{1}{2(\pi \sigma_f T)^2} = \frac{(\text{PRF}/\sigma_f)^2}{19.75}$ \] 
- 这个公式表明： 
- **PRF越高**，改善因子越大（抑制效果越好） 
- **杂波谱展宽 \($\sigma_f$\) 越大**，改善因子越小（抑制效果越差） 
- 图5.9也验证了这一点：更高的PRF会让滤波器的零点更窄，从而更好地抑制集中在零多普勒的杂波。 
![](assets/doc/file-20260203095626084.png)
#### 总结 
这一节定义了杂波衰减和改善因子两个性能指标，其中改善因子更全面；然后通过帕塞瓦尔定理，推导了单延迟对消器的输出杂波功率和改善因子的计算公式，并给出了高斯杂波谱下的近似结果，揭示了PRF和杂波谱展宽对性能的影响。